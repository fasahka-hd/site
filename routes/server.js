...
const ALLOWED_STATIC_EXT = /\.(css|js|png|jpe?g|webp|gif|svg|ico|woff2?|webmanifest)$/i;
...
const staticFilter = (req, file) => {
  const ext = (file.path || "").toLowerCase();
  if (ext.endsWith(".html") || ext.endsWith(".json") || ext.endsWith(".env") || ext.endsWith(".sql") || ext.endsWith(".log") || ext.endsWith(".bak") || ext.endsWith(".orig") || ext.endsWith(".swp")) {
    return false;
  }
  if (!ALLOWED_STATIC_EXT.test(ext) && !ext.endsWith("/")) return false;
  return true;
};
...
const LEGACY_REDIRECTS = new Map([ [ "/index.html", "/" ], [ "/login.html", "/login" ], [ "/bans.html", "/bans" ], [ "/stats.html", "/stats" ], [ "/admin_logs.html", "/admin-logs" ], [ "/blacklist.html", "/blacklist" ], [ "/promos.html", "/promos" ], [ "/zbt_access.html", "/zbt-access" ], [ "/manage.html", "/manage" ], [ "/tech_money.html", "/tech/money" ], [ "/tech_gangs.html", "/tech/gangs" ], [ "/tech_donate.html", "/tech/donate" ], [ "/player.html", "/player" ] ]);

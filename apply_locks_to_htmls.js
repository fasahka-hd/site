...
const NAV_LINK = '  <a class="navItem" data-role-only="KP" href="/manage/locks">Блокировки</a>';
...
function patch(filePath) {
  ...
  const alreadyPatched = src.includes("/manage/locks") || src.includes("locks.html");
  if (src.includes(logoutNeedle) && !alreadyPatched) {
    src = src.replace(logoutNeedle, NAV_LINK + "\n      " + logoutNeedle);
    changed = true;
  } else if (!alreadyPatched || !src.includes('data-role-only="KP"')) {

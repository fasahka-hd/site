...
function bansRoutes() {
  ...
  r.get("/api/bans", authGuard, requirePerm("view_bans"), async (req, res) => {
    try {
      ...
      const [rows] = await pool.query(`SELECT steamid, name, a_name, a_steamid, reason, ban_time, ban_len, unban_time, unban_reason\n         FROM ba_bans ${whereSql} ORDER BY ban_time DESC LIMIT ?, ?`, [ ...params, offset, perPage ]);
      ...
      try {
        ...
        for (const it of items) {
          if ((!String(it.a_name || "").trim() && it.a_steamid64)) missingAdmins.push(it.a_steamid64);
        }
        if (missingAdmins.length) {
          const ph = missingAdmins.map(() => "?").join(",");
          const [adminRows] = await pool.query(`SELECT steamid, name FROM ba_users WHERE steamid IN (${ph}) LIMIT ?`, [...missingAdmins, missingAdmins.length]);
          const adminMap = new Map();
          for (const r of adminRows) {
            const sid = String(r.steamid || "");
            const n = fixCrazyNick(r.name || "");
            if (sid && n) adminMap.set(sid, n);
          }
          for (const it of items) {
            if (!String(it.a_name || "").trim() && it.a_steamid64) {
              const sid32 = it.a_steamid;
              if (adminMap.has(sid32)) it.a_name = adminMap.get(sid32);
              else if (adminMap.has(it.a_steamid64)) it.a_name = adminMap.get(it.a_steamid64);
            }
          }
        }
      } catch (e) {
        console.error("bans admin lookup error:", e.message);
      }
      res.json({
        ok: true,
        items: items,
